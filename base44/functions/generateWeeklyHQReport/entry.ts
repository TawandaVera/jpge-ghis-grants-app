...
Deno.serve(async (req) => {
  try {
    ...
    let emailBody = `JPGE CIE HQ WEEKLY PROGRESS REPORT\nWeek of ${dateRange}\n`;
    ...
    emailBody += `\nThis report was auto-generated by the JPGE Capital Intelligence Engine (JPGE CIE).\nLog in to the platform for full details.`;
    ...
    for (const sub of subscribers) {
      try {
        await base44.asServiceRole.integrations.Core.SendEmail({
          to: sub.email,
          from_name: "JPGE CIE HQ Reports",
          subject: `JPGE CIE HQ Weekly Progress Report — Week of ${weekStart.toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}`,
          body: emailBody,
        });
        sent++;
      } catch (e: any) {
